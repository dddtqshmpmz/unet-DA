...
def parse_args():
    ...
    parser.add_argument('--lambda_mmd_loss', default=0.005, type=float)
    parser.add_argument('--lambda_mmd_adv', default=0.1, type=float)
...
def train(config, train_loader, model, model_D, criterion, criterion_D, criterion_mmd, optimizer, optimizer_D):
    ...
    for source_img, source_mask, target_train_img, target_pseuLabel, _ in train_loader:
        ...
        loss = config['lambda_mmd_adv'] * loss_mmd_value * loss_adv_target
        ...
        for target_val_img, target_val_mask, meta in val_loader:
            ...
            for i in range(len(pred_target)):
                for c in range(config['num_classes']):
                    cv2.imwrite(os.path.join('inputs', config['dataset'], 'pred_target_6k_patches', meta['target_val_id'][i] + '.png'),
...
def pred_target_2k_patches(model_dir):
...
def pred_source_30k_patches(model_dir):
    ...
    # target training & validating img_ids
    source_img_ids = glob(os.path.join(
        'inputs', config['dataset'], 'images_10pics_30k_patches', '*' + '.png'))
    source_img_ids = [os.path.splitext(os.path.basename(p))[
        0] for p in source_img_ids]

    source_train_img_ids = source_img_ids[:]  # 30k
    ...
    val_dataset = TestDataset_DA(
        target_val_ids=source_train_img_ids,
        target_img_dir=os.path.join(
            'inputs', config['dataset'], 'images_10pics_30k_patches'),
        target_mask_dir=os.path.join(
            'inputs', config['dataset'], 'masks_10pics_30k_patches'),
...
def pred_target_6k_patches(model_dir):
    ...
    # target training & validating img_ids
    source_img_ids = glob(os.path.join(
        'inputs', config['dataset'], 'images_30pics_6k_patches', '*' + '.png'))
    source_img_ids = [os.path.splitext(os.path.basename(p))[
        0] for p in source_img_ids]
...
if __name__ == '__main__':
    main()
    # pred_target_2k_patches(model_dir='model_20201212_v4_adam_0.78')
    # pred_source_30k_patches(model_dir='model_20201208_0.87')
    # pred_target_6k_patches(model_dir='model_20201212_v4_adam_0.78')
