...
from models_clan.CLAN_G import Res_Deeplab
from models_clan.CLAN_D import FCDiscriminator
...
from dataset_DA_cvc import Dataset_DA, TestDataset_DA
...
def parse_args():
    ...
    # lambda_adv_target
    parser.add_argument('--lambda_adv_target', default=0.01, type=float)
    parser.add_argument('--lambda_weight', default=0.001, type=float)
...
def train(config, train_loader, model, model_D, criterion, criterion_D, optimizer, optimizer_D):
    ...
    for source_img, source_mask, target_train_img, _ in train_loader:
        ...
        # train with source
        source_pred1, source_pred2 = model(source_img)
        source_pred1 = interp_source(source_pred1)
        source_pred2 = interp_source(source_pred2)
        source_train_loss_1 = criterion(source_pred1, source_mask)
        source_train_loss_2 = criterion(source_pred2, source_mask)

        source_train_dice = dice_coef(source_pred1, source_mask)
        source_train_loss = source_train_loss_1 + source_train_loss_2
        source_train_loss.backward()

        # train with target
        target_train_pred1, target_train_pred2 = model(target_train_img)
        target_train_pred1 = interp_target(target_train_pred1)
        target_train_pred2 = interp_target(target_train_pred2)

        D_out = model_D(
            F.softmax(target_train_pred1 + target_train_pred2, dim=1))
        ...
        # w5 = list(model.segmentation_head[0].parameters())
        W5 = None
        W6 = None
        ...
        loss_weight = loss_weight * config['lambda_weight'] * 2
        ...
        # train with source
        source_pred1 = source_pred1.detach()
        source_pred2 = source_pred2.detach()

        D_out = interp_source(
            model_D(F.softmax(source_pred1 + source_pred2, dim=1)))
        ...
        # train with target
        target_train_pred1 = target_train_pred1.detach()
        target_train_pred2 = target_train_pred2.detach()

        D_out = interp_target(model_D(
            F.softmax(target_train_pred1 + target_train_pred2, dim=1)))
        ...
        for target_val_img, target_val_mask, meta in val_loader:
            ...
            for i in range(len(pred_target)):
                for c in range(config['num_classes']):
                    cv2.imwrite(os.path.join('inputs', 'CVC-ClinicDB', 'pred_target_212_patches', meta['target_val_id'][i] + '.png'),
...
def main():
    ...
    model_D = FCDiscriminator(num_classes=config['num_classes'])
...
def pred_target_212_patches(model_dir):
    ...
    # target training & validating img_ids
    target_img_ids = glob(os.path.join(
        'inputs', 'CVC-ClinicDB', 'images_tif', '*' + '.tif'))
    target_img_ids = [os.path.splitext(os.path.basename(p))[
        0] for p in target_img_ids]
    test_size_ = 1 - config['target_train_num']/612.0
    ...
    val_dataset = TestDataset_DA(
        target_val_ids=target_val_img_ids,
        target_img_dir=os.path.join(
            'inputs', 'CVC-ClinicDB', 'images_tif'),
        target_mask_dir=os.path.join(
            'inputs', 'CVC-ClinicDB', 'masks'),
        img_ext='.tif',
        mask_ext='.jpg',
...
if __name__ == '__main__':
    # main()
    pred_target_212_patches(model_dir='model_20201222_cvc_clan')
