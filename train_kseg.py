...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input, target, meta in val_loader:
            ...
            pred_target = torch.sigmoid(output)
            ...
            for i in range(len(pred_target)):
                for c in range(config['num_classes']):
                    cv2.imwrite(os.path.join('inputs', 'CVC-ClinicDB', 'masks_target_pseudo_labels', meta['img_id'][i] + '.jpg'),
...
def main():
    ...
    train_img_ids, val_img_ids = train_test_split(
        img_ids, test_size=0.2, random_state=41)  # random_state=41
...
def pred_cvc_with_kseg_model():
    ...
    with open('models/%s/%sconfig.yml' % (config['name'], model_dir), 'w') as f:
        yaml.dump(config, f)
    ...
    target_train_img_ids, target_val_img_ids = train_test_split(
        target_img_ids, test_size=0.0, random_state=41)'''
