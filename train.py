...
def parse_args():
    ...
    parser.add_argument('--input_w', default=300, type=int,
                        help='image width')
...
def main():
    ...
    #img_ids2 = glob(os.path.join('inputs', config['dataset'], 'images', '0-2D-2*' + config['img_ext']))

    #img_ids= img_ids1 + img_ids2
    ...
    for epoch in range(config['epochs']):
        ...
        trigger += 1
        if epoch % 10 == 0:
            torch.save(model.state_dict(), 'models/%s/model_0807_epoch_%d.pth' %
                       (config['name'], epoch ))
