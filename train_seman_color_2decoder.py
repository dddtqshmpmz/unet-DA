...
def train(config, train_loader, model, criterion, optimizer):
    ...
    for input,palettes, target, _ in train_loader:
        input = input.cuda()
        palettes = palettes.cuda()
        target = target.cuda()
...
def validate(config, val_loader, model, criterion):
    ...
    with torch.no_grad():
        ...
        for input,palettes, target, _ in val_loader:
            input = input.cuda()
            palettes = palettes.cuda()
            target = target.cuda()
...
def main():
    ...
    model = smp.UnetWithColor('efficientnet-b3', in_channels= 31 ,
    ...
    color_patch_dir = '../../../../home/newDisk/diaoli/results/test_ihc_semantic_color'
    color_patch_dir_parent = '../../../../home/newDisk/diaoli/results'

    # Data loading code
    img_ids = glob(os.path.join(
        color_patch_dir, '*' + config['img_ext']))

    img_ids = [os.path.splitext(os.path.basename(p))[0] for p in img_ids]
    img_ids = [  p.split('_layer')[0]  for p in img_ids]
    img_ids = list(set(img_ids))
    for img_id in img_ids:
        for k in range(0,7):
            if not os.path.exists( color_patch_dir+'/'+ img_id+'_layer-%02d.png' % (k)):
    ...
    train_dataset = DatasetSemanColor2Decoder(
        img_ids=train_img_ids,
        img_dir=os.path.join(
            'inputs', config['dataset'], 'images_10pics_30k_patches'),
        color_img_dir = os.path.join(color_patch_dir_parent,'test_ihc_semantic_color') ,
    ...
    val_dataset = DatasetSemanColor2Decoder(
        img_ids=val_img_ids,
        img_dir=os.path.join(
            'inputs', config['dataset'], 'images_10pics_30k_patches'),
        color_img_dir = os.path.join(color_patch_dir_parent,'test_ihc_semantic_color') ,
