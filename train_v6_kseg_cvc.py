...
from dataset_DA_cvc import Dataset_DA, TestDataset_DA, Dataset_DA_pseuLabel
...
def parse_args():
    ...
    parser.add_argument('--lambda_mmd_adv', default=0.2, type=float)
        ...
        for target_val_img, target_val_mask, meta in val_loader:
            ...
            for i in range(len(pred_target)):
                for c in range(config['num_classes']):
                    size_img = cv2.imread(
                        './inputs/CVC-ClinicDB/masks/' + meta['target_val_id'][i] + '.jpg', 0)
                    mask_pred = (pred_target[i, c] * 255).astype('uint8')
                    mask_pred = cv2.resize(
                        mask_pred, (size_img.shape[1], size_img.shape[0]))
                    cv2.imwrite(os.path.join(
                        'inputs', 'CVC-ClinicDB', 'pred_target_212_patches', meta['target_val_id'][i] + '.jpg'), mask_pred)
...
def main():
    ...
    # 改数量
    source_train_img_ids = source_img_ids[:config['source_train_num']]
    ...
    train_dataset = Dataset_DA_pseuLabel(
        ...
        target_pseuLabel_dir=os.path.join(
            'inputs', 'CVC-ClinicDB', 'masks_target_pseudo_labels_0.732'),
...
def pred_target_212_patches(model_dir):
    ...
    model.load_state_dict(torch.load(
        'models/%s/%s/model_epoch_20.pth' % (config['name'], model_dir)))
    model.eval()
...
if __name__ == '__main__':
    # main()
    pred_target_212_patches(model_dir='model_20201224_cvc_color_3')
