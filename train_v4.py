...
def parse_args():
    ...
    # lambda_adv_target
    parser.add_argument('--lambda_adv_target', default=0.07, type=float)
    parser.add_argument('--lambda_target_train_pseu_loss',
                        default=0.01, type=float)
    parser.add_argument('--source_train_num', default=16000, type=int)
...
def train(config, train_loader, model, model_D, criterion, criterion_D, optimizer, optimizer_D):
    ...
    for source_img, source_mask, target_train_img, target_pseuLabel, _ in train_loader:
        ...
        loss = config['lambda_target_train_pseu_loss'] * target_train_pseu_loss
        loss_pseu = target_train_pseu_loss.item()
    ...
    return OrderedDict([('source_train_loss', avg_meters['source_train_loss'].avg), ('source_train_dice', avg_meters['source_train_dice'].avg),
                        ('loss_D', avg_meters['loss_D'].avg), ('loss_adv', avg_meters['loss_adv'].avg), ('loss_pseu', avg_meters['loss_pseu'].avg)])
...
def main():
    ...
    for epoch in range(config['epochs']):
        ...
        print('source_train_loss %.4f - source_train_dice %.4f - loss_pseu %.4f - loss_D %.4f - loss_adv %.4f - target_val_loss %.4f - target_val_dice %.4f'
              % (train_log['source_train_loss'], train_log['source_train_dice'], train_log['loss_pseu'], train_log['loss_D'], train_log['loss_adv'], val_log['target_val_loss'], val_log['target_val_dice']))
...
def pred_target_1k_patches(model_dir):
    ...
    model.load_state_dict(torch.load(
        'models/%s/%s/model_epoch_10.pth' % (config['name'], model_dir)))
    model.eval()
...
if __name__ == '__main__':
    # main()
    pred_target_1k_patches(model_dir='model_20201213_v4_trainNum')
